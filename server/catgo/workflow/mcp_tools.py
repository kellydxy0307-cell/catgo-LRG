...
def get_tool_definition() -> dict:
    ...
    return {
        ...
        "description": (
            ...
            "ALSO confirm the pseudopotential/POTCAR directory for that cluster: if you are "
            "not certain where POTCAR/pseudopotential files live (potcar_root etc.), STOP and "
            "ASK THE USER -- do NOT guess (a wrong path fails every job; it is per-user/"
            "per-cluster and not inferable from another workflow). On Expanse POTCAR can be "
            "generated via 'echo -e 103 | vaspkit'. "
